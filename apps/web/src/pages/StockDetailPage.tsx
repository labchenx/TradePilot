...
import type { StockDetailTrendRange } from '@/types';
import { formatCurrency } from '@/utils';

const trendRanges: StockDetailTrendRange[] = ['1M', '3M', '6M', '1Y'];
const symbolColors = [
  '#10b981',
  '#3b82f6',
  '#8b5cf6',
...
function symbolColor(symbol: string) {
  ...
  return symbolColors[score % symbolColors.length];
}
...
function formatTrendDate(value: string) {
  ...
  const options: Intl.DateTimeFormatOptions = value.endsWith('-01')
    ? { month: 'short' }
...
export function StockDetailPage() {
  ...
  const [trendRange, setTrendRange] =
    useState<StockDetailTrendRange>('6M');
  const { data, loading, error } = useStockDetail(
    normalizedSymbol,
    trendRange,
  );
  const holding = data?.holding;
  const relatedTrades = data?.transactions ?? [];
  const priceHistory = data?.priceTrend ?? [];
  const relatedNews = mockNews
    .filter((news) => news.symbol === normalizedSymbol)
    .slice(0, 3);
  const currency =
    holding?.currency ??
    priceHistory[0]?.currency ??
    relatedTrades[0]?.currency ??
    'USD';
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link
          to="/holdings"
          className="mb-4 inline-flex items-center text-sm text-neutral-500 transition-colors hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white"
        >
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div className="flex items-center gap-4">
            <div
              ...
              style={{ backgroundColor: symbolColor(normalizedSymbol) }}
            >
            ...
            <div>
              <h2 className="flex items-center gap-3 text-3xl font-bold text-neutral-900 dark:text-white">
                {normalizedSymbol}
                <Tag color={holding ? 'green' : 'gray'}>
                  {holding ? 'Active Holding' : 'No Active Holding'}
                </Tag>
              </h2>
              <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                {holding?.name ?? normalizedSymbol}
              </p>
            </div>
          </div>
          <div className="flex flex-col items-end">
            <div className="text-3xl font-bold text-neutral-900 dark:text-white">
              {nullableCurrency(holding?.marketPrice, currency)}
            </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <Metric
          label="Market Value"
          value={nullableCurrency(holding?.marketValue, currency)}
        />
        <Metric
          label="Holding Shares"
          value={holding ? holding.quantity.toString() : '--'}
        />
        <Metric
          label="Avg Cost"
          value={nullableCurrency(holding?.avgCost, currency)}
        />
          ...
          <CardShell className="p-5">
            ...
            <div className="h-72">
              {priceHistory.length === 0 ? (
                <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-neutral-200 bg-neutral-50 text-sm text-neutral-500 dark:border-neutral-800 dark:bg-neutral-900/40 dark:text-neutral-400">
                  No cached price history for {normalizedSymbol}.
                </div>
              ) : (
                ...
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {relatedTrades.length === 0 ? (
                    <tr>
                      ...
                      >
                        No imported trades for {normalizedSymbol}.
                      </td>
                    </tr>
                  ) : (
                    relatedTrades.map((trade) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-3 text-right tabular-nums text-neutral-600 dark:text-neutral-300">
        ...
        <div className="space-y-6 lg:col-span-1">
          <CardShell className="bg-gradient-to-b from-blue-50 to-white p-5 dark:from-blue-950/20 dark:to-neutral-900">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Button
                size="sm"
                variant="secondary"
                className="h-8 border-0 bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/50 dark:text-blue-300"
              >
                Regenerate
              </Button>
            </div>
            <p className="mb-4 text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">
              This is a static information organization placeholder for{' '}
              {normalizedSymbol}. It does not constitute investment advice.
            </p>
            <div className="mb-6 space-y-3">
              <Insight
                icon={Lightbulb}
                title="Positive Factors"
                text="Strong portfolio contribution and current momentum in mock data."
                tone="green"
              />
              <Insight
                icon={TrendingDown}
                title="Negative Factors"
                text="Monitor valuation and concentration risk before adding exposure."
                tone="red"
              />
              <Insight
                icon={Target}
                title="Watch Items"
                text="Review future earnings and import accuracy when backend data is connected."
                tone="yellow"
              />
            </div>
            <div className="flex gap-2 rounded-lg bg-blue-100/50 p-3 dark:bg-blue-900/20">
              ...
              <p className="text-xs leading-relaxed text-blue-800 dark:text-blue-300/80">
                AI summary is for information organization only. 不构成投资建议。
              </p>
            </div>
          </CardShell>
          ...
          <CardShell className="p-5">
            ...
            <div className="space-y-4">
              {(relatedNews.length ? relatedNews : mockNews.slice(0, 3)).map(
                (news) => (
...
function Insight({
  ...
  return (
    <div className="flex gap-3 text-sm">
      ...
      <div>
        <span className={`mb-0.5 block font-semibold ${styles[tone]}`}>
          {title}
        </span>
